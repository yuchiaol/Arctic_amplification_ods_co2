...
#   plt.plot([4]*10000,xx,'.', color='gold', alpha=0.05)
   ...
   data = np.concatenate((arctic_sat_ctr_change,arctic_sat_xc2_change,arctic_sat_xod_change,arctic_sat_xco_change))
   ...
   for II in range(ens_num):
       ...
       plt.plot([3]*2,[arctic_sat_xod_change[II]]*2,'bo',markersize=3)
       plt.plot([2]*2,[arctic_sat_xc2_change[II]]*2,'ro',markersize=3)
       plt.plot([4]*2,[arctic_sat_xco_change[II]]*2,'go',markersize=3)
   plt.plot([1]*2,[arctic_sat_ctr_change.mean()]*2,'ko',markersize=10)
   plt.plot([3]*2,[arctic_sat_xod_change.mean()]*2,'bo',markersize=10)
   plt.plot([2]*2,[arctic_sat_xc2_change.mean()]*2,'ro',markersize=10)
   plt.plot([4]*2,[arctic_sat_xco_change.mean()]*2,'go',markersize=10)
   plt.title('(a) Annual Arctic SAT trend', fontsize=12)
   plt.xticks([1,2,3,4],['ALL','xCO2','xODS','xCO2&ODS'],rotation=0)
...
#   plt.xlim(0.5,5.5)
   ...
   xx = np.nanmean(arctic_sat_xod_change_resample, axis=1)
   sns.distplot(xx, hist=True, kde=True, color='c', label='xODS')
   xx = np.nanmean(arctic_sat_xc2_change_resample, axis=1)
   sns.distplot(xx, hist=True, kde=True, color='m', label='xCO2')
   xx = np.nanmean(arctic_sat_xco_change_resample, axis=1)
   sns.distplot(xx, hist=True, kde=True, color='g', label='xCO2&ODS')
...
#   plt.plot([4]*10000,xx*factor,'.', color='gold', alpha=0.05)
   plt.plot([0.5,4.5],[0,0],'k-')
   data = np.concatenate((arctic_sie_ctr_change*factor,arctic_sie_xc2_change*factor,arctic_sie_xod_change*factor,arctic_sie_xco_change*factor))
   data = data.reshape((int(len(data)/ens_num),ens_num)).T
   plt.boxplot(data, flierprops=flierprops, whis='range')
   for II in range(ens_num):
       plt.plot([1]*2,[arctic_sie_ctr_change[II]*factor]*2,'ko',markersize=3)
       plt.plot([3]*2,[arctic_sie_xod_change[II]*factor]*2,'bo',markersize=3)
       plt.plot([2]*2,[arctic_sie_xc2_change[II]*factor]*2,'ro',markersize=3)
#       plt.plot([4]*2,[arctic_sie_xoo_change[II]*factor]*2,'yo',markersize=3)
       plt.plot([4]*2,[arctic_sie_xco_change[II]*factor]*2,'go',markersize=3)
   plt.plot([1]*2,[arctic_sie_ctr_change.mean()*factor]*2,'ko',markersize=10)
   plt.plot([3]*2,[arctic_sie_xod_change.mean()*factor]*2,'bo',markersize=10)
   plt.plot([2]*2,[arctic_sie_xc2_change.mean()*factor]*2,'ro',markersize=10)
#   plt.plot([4]*2,[arctic_sie_xoo_change.mean()*factor]*2,'yo',markersize=10)
   plt.plot([4]*2,[arctic_sie_xco_change.mean()*factor]*2,'go', markersize=10)
...
#   plt.hist(xx, bins=25, color='r',alpha=0.3, rwidth=0.6, label='Hist')
   xx = np.nanmean(arctic_sie_xod_change_resample, axis=1)*factor
   sns.distplot(xx, hist=True, kde=True, color='c', label='xODS')
#   plt.hist(xx, bins=25, color='b',alpha=0.3, rwidth=0.6, label='ODS')
   xx = np.nanmean(arctic_sie_xc2_change_resample, axis=1)*factor
   sns.distplot(xx, hist=True, kde=True, color='m', label='xCO2')
   xx = np.nanmean(arctic_sie_xco_change_resample, axis=1)*factor 
   sns.distplot(xx, hist=True, kde=True, color='g', label='xCO2&ODS')
