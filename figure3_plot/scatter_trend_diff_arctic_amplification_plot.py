...
# read observational estimate
   filename = 'obs_le_trend_tmp.nc'
   f = Dataset(filename, 'r')
   ts_global_obs = f.variables['ts_global_trend'][0:50].data
   ts_arctic_obs = f.variables['ts_arctic_trend'][0:50].data
   f.close()
...
#   plt.plot([1]*2,[aa_sat_obs_trend.mean()]*2,'ro')
   plt.plot([1.25]*50,ts_arctic_obs/ts_global_obs,'go',markersize=3, alpha=0.5)
   plt.plot([1.25],(ts_arctic_obs/ts_global_obs).mean(),'go',markersize=10, label='OBS (' + str(round((ts_arctic_obs/ts_global_obs).mean(), 2))  + ')')
...
#   p_xod = np.polyfit(global_sat_ctr_trend-global_sat_xod_trend,arctic_sat_ctr_trend-arctic_sat_xod_trend,1)
   ...
   ts_x = ts_global_obs.copy()
   ts_y = ts_arctic_obs.copy()
   p_obs_res = stats.theilslopes(ts_y,ts_x)
   p_obs = np.polyfit(ts_x,ts_y,1)
#   text2 = 'OBS, slope=' + str(round(p_obs_res[0],2)) + '$\pm$' + str(round(abs(p_obs_res[0]-p_obs_res[2]), 2))
   text2 = 'OBS, slope=' + str(round(p_obs_res[0],2))
   plt.plot(ts_global_obs.mean(),ts_arctic_obs.mean(),'go', markersize=14, label=text2)
   plt.plot(ts_global_obs,ts_arctic_obs,'go', markersize=3, alpha=0.5)
#   p_xc2 = np.polyfit(global_sat_ctr_trend-global_sat_xc2_trend,arctic_sat_ctr_trend-arctic_sat_xc2_trend,1)
#   plt.plot([-5,5],[p_xc2[0]*-5+p_xc2[1],p_xc2[0]*5+p_xc2[1]],'g-', linewidth=0.5)
#   plt.plot(np.array([-5,5]), p_xc2_res[1] + p_xc2_res[2]*np.array([-5,5]), 'g--', linewidth=0.5)
#   plt.plot(np.array([-5,5]), p_xc2_res[1] + p_xc2_res[3]*np.array([-5,5]), 'g--', linewidth=0.5)

   plt.axis([0,0.15,0.0,0.42])
